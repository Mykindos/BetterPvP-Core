...
public class BuildUnlockCommand extends Command {
    ...
    public BuildUnlockCommand() {
        super("b-unlock", new String[]{}, Rank.ADMIN);
    }
    ...
    public void execute(Player player, String[] args) {
        ...
        if (args.length == 1) {
            ...
            Punish punish = PunishManager.getPunish(target.getUUID(), PunishType.BuildLock);
            if (punish == null) {
                UtilMessage.message(player, "Punish", ChatColor.YELLOW + target.getName() + ChatColor.GRAY + " has not been Punished.");
                return;
            }

            if (!PunishManager.isBuildLocked(target.getUUID())) {
