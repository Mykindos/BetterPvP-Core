...
public class UnbanCommand extends Command {
    ...
    public UnbanCommand() {
        super("unban", new String[]{}, Rank.ADMIN);
    }
    ...
    public void execute(Player player, String[] args) {
        ...
        if (args.length == 1) {
            ...
            Punish punish = PunishManager.getPunish(target.getUUID(), PunishType.BAN);
            if (punish == null) {
                UtilMessage.message(player, "Punish", ChatColor.YELLOW + target.getName() + ChatColor.GRAY + " has not been Punished.");
                return;
            }

            if (!PunishManager.isBanned(target.getUUID())) {
                UtilMessage.message(player, "Punish", ChatColor.YELLOW + target.getName() + ChatColor.GRAY + " is not banned.");
                return;
            }

            PunishRepository.removePunishment(punish);
            PunishManager.removePunishment(punish);
            UtilMessage.broadcast("Punish", ChatColor.YELLOW + player.getName() + ChatColor.GRAY + " unbanned " + ChatColor.YELLOW + target.getName()
                    + ChatColor.GRAY + ".");
        	Log.write("Punish", player.getName() + " unbanned " +
					 target.getName());
